...
export class Scene_MathEquation {
  ...
  buildTimeline(params) {
    ...
    tl.set(this.root, { autoAlpha: 1 }, 0)
      .fromTo(this.equation, { y: 28, autoAlpha: 0, scale: .94 }, { y: 0, autoAlpha: 1, scale: 1, duration: .85, ease: 'power3.out' }, .18);
    if (params.highlight) {
      tl.fromTo(this.highlight, { autoAlpha: 0, scale: .75 }, { autoAlpha: 1, scale: 1, duration: .55, ease: 'back.out(1.6)' }, .92)
        .to(this.highlight, { boxShadow: '0 0 55px rgba(255,210,63,.2)', duration: .7, yoyo: true, repeat: 1 }, 1.42);
    }
    tl.fromTo(this.rule, { scaleX: 0 }, { scaleX: 1, duration: .55, ease: 'power2.inOut' }, 1.42)
      .fromTo(this.caption, { y: 18, autoAlpha: 0 }, { y: 0, autoAlpha: 1, duration: .65 }, 1.62);
